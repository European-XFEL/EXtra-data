...
class KeyData:
    ...
    def train_from_id(self, tid, keepdims=False):
    ...
    def train_from_index(self, i, keepdims=False):
        """Get data for a train by index (starting at 0) as a numpy array.

        Returns (train ID, array)
        """
        return self.train_from_id(self.train_ids[i], keepdims=keepdims)

    def trains(self, keepdims=False):
